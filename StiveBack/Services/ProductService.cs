...
namespace StiveBack.Services
{
    public class ProductService
    {
        ...
        {
            Product product = _database.products.Include(p => p.CategoryId).ThenInclude(pc => pc.Category).FirstOrDefault(p => p.Id == id);
        ...
        public List<ProductRessource> SelectAll()
        {
            ...
            List<Product> allProducts = _database.products.Include(p => p.CategoryId).ThenInclude(pc => pc.Category).ToList();
        ...
        {
            Product product = _database.products.Include(p => p.CategoryId).FirstOrDefault(p => p.Id == id);
            if (product == null)
            {
                return null;
            }
            ...
            product.CategoryId.Clear();

            foreach (var categoryId in productSaveRessource.CategoryIds)
            {
                product.CategoryId.Add(new ProductCategory
                {
        ...
        public Product ProductSaveRessourceToProduct(ProductSaveRessource productSaveRessource)
        {
            ...
            var product = new Product
            {
                ...
                CategoryId = productSaveRessource.CategoryIds.Select(categoryId => new ProductCategory
                {
                    CategoryId = categoryId,
                }).ToList(),
            };
        ...
        public ProductRessource ProductToProductRessource(Product product)
        {
            ...
            ProductRessource productRessource = new ProductRessource
            {
                ...
                ProductCategories = product.CategoryId
            .Where(pc => pc.Category != null)
            .Select(pc => new CategoryRessource
            {
                Id = pc.Category.Id,
                Name = pc.Category.Name,
                CategoryRessourceParentId = pc.Category.CategoryParentId
            }).ToList(),
